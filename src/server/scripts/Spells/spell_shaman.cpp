...
class spell_sha_flametongue_weapon : public AuraScript
{
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
    {
        ...
        if (bot && bot->IsNPCBot())
        {
            ...
            spellPowerBonus += eventInfo.GetActionTarget()->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_DAMAGE_TAKEN, SPELL_SCHOOL_MASK_FIRE);
            ...
            bot->CastSpell(eventInfo.GetActionTarget(), SPELL_SHAMAN_FLAMETONGUE_ATTACK, args);
            return;
        }
        //end npcbot
